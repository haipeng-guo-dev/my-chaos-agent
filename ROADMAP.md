...
```mermaid
flowchart TD
    subgraph TargetApp["TARGET SPRING BOOT JAR"]
        App["Spring Cloud App / Tomcat / Netty"]
    end

    subgraph ChaosAgent["YOUR CHAOS JAVA AGENT"]
        Instrumentation["Byte Buddy / ASM (Instrumentation)"]
        HttpServer["Isolated Embedded Server (com.sun.net.httpserver.HttpServer)"]
        Dashboard["HTML5 / Tailwind Dashboard (Served directly by Agent)"]
    end

    App <-- "Altered Bytes" --> Instrumentation
...
- [x] **Shaded Packaging Pipeline:** Configure build plugins (Maven Shade / Gradle Shadow) to isolate and relocate internal packages (e.g., Byte Buddy) to safeguard host classpaths.
